...
// ========== OM Positions Display on Lighter (grouped by symbol) ==========
let cachedOMPositions: Position[] = []

function updateOMPositionsOnLighter(positions: Position[]) {
    if (SITE_TYPE !== 'lighter') return
...
function createOMPositionRow(pos: Position, _lgIndex: number, _omIndex: number, isOrphan = false): HTMLElement {
...
function startLGPositionsObserver() {
    if (SITE_TYPE !== 'lighter') return

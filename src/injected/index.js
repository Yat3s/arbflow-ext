;(function () {
  ...
  if (window.__ARBFLOW_INJECTED__) {
    console.log('[Arbflow] Injected script already loaded, skipping')
    return
  }
  window.__ARBFLOW_INJECTED__ = true
  ...
  const arbflowWs = {
    connect: function (url, options) {
      ...
      const exchange = id.startsWith('lighter-')
        ? 'lighter'
        : id.startsWith('omni-')
          ? 'omni'
          : null
  ...
  var arbflowTrade = {
    executeOmniTrade: function (params) {
      ...
      console.log(
        '[Arbflow] 🔑 walletAddress from params:',
        walletAddress,
        'type:',
      ...
      if (walletAddress) {
        headers['vr-connected-address'] = walletAddress
        console.log('[Arbflow] ✅ Added vr-connected-address header:', walletAddress)
      } else {
        console.log('[Arbflow] ⚠️ No walletAddress provided, skipping vr-connected-address header')
      }

      console.log('[Arbflow] 📤 Requesting quote:', quoteBody)
      console.log('[Arbflow] 📤 Request headers:', headers)
